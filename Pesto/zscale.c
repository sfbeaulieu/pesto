...
void
cdl_zscale (
    unsigned short int *im,		/* image data to be sampled		*/
    int nx,
...
//#endif
{
    ...
    /* Subsample the image. */
    npix = sampleImage((unsigned short int *)im, bitpix, &sample, nx, ny,
...
unsigned short int	*im;		/* image to be sampled			*/
int	bitpix;			/* bits per pixel in image		*/
float	**sample;		/* output vector containing the sample	*/
int	nx, ny;			/* image dimensions			*/
int	optimal_size;		/* desired number of pixels in sample	*/
...
#endif
{
    ...
    *sample = (float *) malloc (maxpix * sizeof (float));
    ...
    for (line = (line_step + 1)/2; line < nlines; line+=line_step) {
        ...
        spix = (unsigned short *) &im[(line-1) * nx ];
                for (i=0; i < nx; i++)
            row[i] = (float) spix[i];
                break;
            case 32:
